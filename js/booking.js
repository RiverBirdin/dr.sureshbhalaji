...
function initBookingForm() {
  ...
  function buildWhatsAppMessage() {
    ...
    const bookingName = siteConfig.booking?.whatsappName || "clinic team";

    return (
      `Hello ${bookingName}, I would like to request a consultation with ${siteConfig.doctor.displayName}.\n\n` +
  ...
  initBookingWhatsAppInfo();

  function initBookingWhatsAppInfo() {
    const { booking } = siteConfig;
    if (!booking?.whatsappName) return;

    document.querySelectorAll("[data-booking-whatsapp-name]").forEach((el) => {
      el.textContent = booking.whatsappName;
    });
    document.querySelectorAll("[data-booking-whatsapp-number]").forEach((el) => {
      el.textContent = booking.whatsappDisplay || booking.whatsapp.replace(/\D/g, "").replace(/^91/, "");
    });
    document.querySelectorAll("[data-booking-whatsapp-link]").forEach((el) => {
      const num = booking.whatsapp.replace(/\D/g, "");
  ...
  form.addEventListener("submit", (e) => {
    ...
    const bookingWhatsApp =
      siteConfig.booking?.whatsapp || siteConfig.contact.whatsapp;
    const message = encodeURIComponent(buildWhatsAppMessage());

    if (bookingWhatsApp.startsWith("[")) {
      if (alertSuccess) {
      ...
      const cleanNumber = bookingWhatsApp.replace(/\D/g, "");
      window.open(
